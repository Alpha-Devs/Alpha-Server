...
Config.chatfilter = function (message, user, room, connection) {
	...
	if (!user.can('hotpatch') && (pre_matches >= 1 && final_check == 0 || pre_matches >= 2 && final_check >= 1 || message.match(adRegex))) {
		...
		if (user.advWarns > 1) {
			...
			let alertMessage =
			Misc.pmAllStaff(user.name + " has been locked for attempting to advertise" + (room ? ". **Room:**" + room.id : " in a private message.") + " **Message:** " + message, "~Server");
			Rooms('staff').add('|raw|' + Misc.nameColor(user.name, false) + " has been locked for attempting to advertise" + (room ? ". <b>Room:</b> " + room.id : " in a private message.") + " <b>Message:</b> " + message).update();
			return false;
		}
		Misc.pmAllStaff(user.name + " has attempted to advertise" + (room ? ". **Room:** " + room.id : " in a private message.") + " **Message:** " + message, "~Server");
		Rooms('staff').add('|raw|' + Misc.nameColor(user.name, false) + " has attempted to advertise" + (room ? ". <b>Room:</b> " + room.id : " in a private message.") + " <b>Message:</b> " + message).update();
