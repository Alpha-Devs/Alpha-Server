/**
  * Title plugin for showdown that appears in the profile while doing /profile
   * Plugin developed by ~RichardAdams
   **/
'use strict';
...
/*
**Title Plugin Created by ~RichardAdams
*/
...
exports.commands = {
	...
	resettitle: function (target, room, user) {
		if (!this.can('forcewin')) return false;
		...
		let title = parts.slice(1).join(", ").trim();
		Db('about').set(username, title);
		this.sendReply(target + " title have been removed");
		logTitle(user.name + " removed the Title of " + target + ".");
	},
	resettitlehelp: ["/resettitle [user] - Reset user's title"],
